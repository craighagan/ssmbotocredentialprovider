...
CREDENTIAL_REFRESH_TIME = datetime.timedelta(minutes=5)
...
PING_PATH = "/ping"
PING_RESPONSE = "pong"
INSTANCE_DOCUMENT_OVERRIDE_FILE = os.path.join(OVERRIDE_PATH, "instance_document_overrides.json")
...
class FakeMetadataCredentialProvider(object):
    def __init__(self, credential_file=CREDENTIAL_FILE,
                       ssm_registration_file=SSM_REGISTRATION_FILE,
                       credential_refresh_time=CREDENTIAL_REFRESH_TIME):
        self._metadata = {}
    ...
    def metadata(self):
        if not self._metadata:
            registration_data = json.loads(open(self.ssm_registration_file).read())
            ses = boto3.Session()
            data = ses.client("sts").get_caller_identity()
            self._metadata = {
    ...
    def get_credentials(self):
        ...
        config.read(self.credential_file)
        # don't know expiration set to 5m
        exp_date = (datetime.datetime.utcnow() + self.credential_refresh_time).strftime(botocore.auth.ISO8601)
        result = {
            "accessKeyId": config['default']['aws_access_key_id'],
            "secretAccessKey": config['default']['aws_secret_access_key'],
            "sessionToken": config['default']['aws_session_token'],
            "expiration": exp_date,
            }
        return result
